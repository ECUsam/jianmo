...
def get_user_start_loc(train, test):
    ...
    user_sloc_train = pd.read_csv('output_train.csv')[['userid', 'geohashed_start_loc']].drop_duplicates()
    user_sloc_test = pd.read_csv('output_test.csv')[['userid', 'geohashed_start_loc']].drop_duplicates()
...
def get_bike_next_sloc(train, test):
    train_set = pd.read_csv('output_train.csv')
    test_set = pd.read_csv('output_test.csv')
...
def get_sample(train, test, load=False):
    ...
    print(load)
    if load:
        sample_27 = pd.read_pickle('save/sample_filter_{}.pickle'.format(test.shape[0]))[['orderid', 'geohashed_end_loc', 'userid', 'bikeid', 'biketype', 'starttime', 'geohashed_start_loc']]
