...
def get_loc_filter(train, result):
    ...
    for i in tqdm.tqdm(result[['geohashed_start_loc', 'geohashed_end_loc']].drop_duplicates().values):
        ...
        if (i[0] in sloc_elocs) and (i[1] in eloc_slocs):
            ...
            for common_conn1 in sloc_eloc_common_conn1_set:
                common_conn1_count = common_conn1_count + sloc_elocs[i[0]][common_conn1] + eloc_slocs[i[1]][
                    common_conn1]
        ...
        if (i[0] in eloc_slocs) and (i[1] in sloc_elocs):
            ...
            for common_conn2 in sloc_eloc_common_conn2_set:
                common_conn2_count = common_conn2_count + eloc_slocs[i[0]][common_conn2] + sloc_elocs[i[1]][
                    common_conn2]
        sloc_eloc_common_conn2_count.append(common_conn2_count)
    loc_filter = pd.DataFrame({"geohashed_start_loc": sloc_list, "geohashed_end_loc": eloc_list,
                               "sloc_eloc_common_eloc_count": sloc_eloc_common_eloc_count,
                               "sloc_eloc_common_sloc_count": sloc_eloc_common_sloc_count,
                               "sloc_eloc_common_conn1_count": sloc_eloc_common_conn1_count,
                               "sloc_eloc_common_conn2_count": sloc_eloc_common_conn2_count,
                               })
    ...
    result['sloc_eloc_common_eloc_rate'] = result['sloc_eloc_common_eloc_count'] / (
            result['sloc_count'] + result['eloc_as_sloc_count'])
    result['sloc_eloc_common_sloc_rate'] = result['sloc_eloc_common_sloc_count'] / (
            result['sloc_as_eloc_count'] + result['eloc_count'])
    result['sloc_eloc_common_conn1_rate'] = result['sloc_eloc_common_conn1_count'] / (
            result['sloc_count'] + result['eloc_count'])
    result['sloc_eloc_common_conn2_rate'] = result['sloc_eloc_common_conn2_count'] / (
            result['sloc_as_eloc_count'] + result['eloc_as_sloc_count'])
    return result
...
def get_user_loc_filter(train, result):
    ...
    for i in tqdm.tqdm(result[['userid', 'geohashed_start_loc', 'geohashed_end_loc']].drop_duplicates().values):
        ...
        if (i[0] in user_sloc_elocs) and (i[1] in user_sloc_elocs[i[0]]) and (i[2] in user_sloc_elocs[i[0]]):
            ...
            for user_common_eloc in user_sloc_eloc_common_eloc_set:
                user_common_eloc_count = user_common_eloc_count + user_sloc_elocs[i[0]][i[1]][user_common_eloc] + \
                                         user_sloc_elocs[i[0]][i[2]][user_common_eloc]
        ...
        if (i[0] in user_eloc_slocs) and (i[1] in user_eloc_slocs[i[0]]) and (i[2] in user_eloc_slocs[i[0]]):
            ...
            for user_common_sloc in user_sloc_eloc_common_sloc_set:
                user_common_sloc_count = user_common_sloc_count + user_eloc_slocs[i[0]][i[1]][user_common_sloc] + \
                                         user_eloc_slocs[i[0]][i[2]][user_common_sloc]
        user_sloc_eloc_common_sloc_count.append(user_common_sloc_count)
        # 获取地址对在用户历史记录中共有的连接点数目(出发点->xx->目的地)
        user_common_conn1_count = 0
        if (i[0] in user_sloc_elocs) and (i[1] in user_sloc_elocs[i[0]]) and (i[0] in user_eloc_slocs) and (
                i[2] in user_eloc_slocs[i[0]]):
            ...
            for user_common_conn1 in user_sloc_eloc_common_conn1_set:
                user_common_conn1_count = user_common_conn1_count + user_sloc_elocs[i[0]][i[1]][user_common_conn1] + \
                                          user_eloc_slocs[i[0]][i[2]][user_common_conn1]
        user_sloc_eloc_common_conn1_count.append(user_common_conn1_count)
        # 获取地址对在用户历史记录中共有的连接点数目(出发点<-xx<-目的地)
        user_common_conn2_count = 0
        if (i[0] in user_eloc_slocs) and (i[1] in user_eloc_slocs[i[0]]) and (i[0] in user_sloc_elocs) and (
                i[2] in user_sloc_elocs[i[0]]):
            ...
            for user_common_conn2 in user_sloc_eloc_common_conn2_set:
                user_common_conn2_count = user_common_conn2_count + user_eloc_slocs[i[0]][i[1]][user_common_conn2] + \
                                          user_sloc_elocs[i[0]][i[2]][user_common_conn2]
        user_sloc_eloc_common_conn2_count.append(user_common_conn2_count)


    user_loc_filter = pd.DataFrame(
        {"userid": user_list, "geohashed_start_loc": user_sloc_list, "geohashed_end_loc": user_eloc_list,
         "user_sloc_eloc_common_eloc_count": user_sloc_eloc_common_eloc_count,
         "user_sloc_eloc_common_sloc_count": user_sloc_eloc_common_sloc_count,
         "user_sloc_eloc_common_conn1_count": user_sloc_eloc_common_conn1_count,
    ...
    result['user_sloc_eloc_common_sloc_rate'] = result['user_sloc_eloc_common_sloc_count'] / (
            result['user_sloc_as_eloc_count'] + result['user_eloc_count'])
    result['user_sloc_eloc_common_conn1_rate'] = result['user_sloc_eloc_common_conn1_count'] / (
            result['user_sloc_count'] + result['user_eloc_count'])
    result['user_sloc_eloc_common_conn2_rate'] = result['user_sloc_eloc_common_conn2_count'] / (
